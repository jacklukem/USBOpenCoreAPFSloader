...
sudo osascript -e 'display notification "USB preparing please wait a couple of minutes" with title "USBOpenCoreAPFSloader4s" subtitle "Do not remove the USB drive during this phase"'
echo "\nInstalling the bootloader to its boot path"
sudo unzip -qo -P jackluke /private/tmp/USBOpenCoreAPFSloader4s/OpenCore -d /Volumes/EFIext
sudo bless --folder /Volumes/EFIext/EFI/Boot --label "OpenCoreAPFSloader4s"
sudo osascript -e 'display notification "USBOpenCoreAPFSloader4s is ready" with title "USBOpenCoreAPFSloader4s"'
echo "\nDone"
